...
- **Issue: With Cloud Connector Edition 2.1, when running Install-CcAppliance, you receive an error message such as: “Failed to install new instance with error: Cannot set "State" because only strings can be used as values to set XmlNode properties”
    ...
    **Resolution:** In cloudconnector.ini, under section [Common], please add the “State” config, as below:

CountryCode=US
State=WA
City=Redmond

It is not mandatory for the “State” line to have value, however the “State=” line cannot be removed from the cloudconnector.ini.
